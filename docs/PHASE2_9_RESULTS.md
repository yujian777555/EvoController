...
## 补充探针：极端动作对比假设被否证（零新算力）

PHASE2_9_STATUS.md 与 Phase 2.8 的后续建议中都把"候选池对比度不足"列为**唯一未被证否的杠杆**
（假设：同状态下 12 个随机采样动作彼此太接近，导致可区分性低）。**该假设已用现有 1200 状态
数据直接检验并否证。**

方法：对 h=20 的每个状态，枚举所有"结果不同"的候选配对（n = **43,913** 对），
计算动作对比度

```
contrast(a,b) = hypot(|Δlog multiplier|, |Δlog exploration|) + [operator 不同]
```

（log 尺度，因 multiplier 与 exploration 均跨越约 1 个数量级），
再按 contrast 五分位统计**结果差异幅度** |Δy|：

| contrast 五分位 | 配对数 | \|Δy\| 中位 |
|---|---|---|
| [0.013, 1.382) | 8,783 | 0.01937 |
| [1.382, 2.328) | 8,782 | 0.02144 |
| [2.328, 5.207) | 8,783 | 0.02084 |
| [5.207, 6.510) | 8,782 | 0.02085 |
| [6.510, 9.202) | 8,783 | 0.01947 |

**Spearman(contrast, |Δy|) = −0.0034 ≈ 0。**

**结论**：**动作在参数空间相差多少，与它们的结果相差多少完全无关。**
对比度最高的五分位（contrast 6.5–9.2，即几乎相反的动作）与最低的五分位
（contrast 0.013–1.38，几乎相同的动作）产生**同样幅度**的结果差异。

这有三个重要含义：

1. **"极端动作对比"探针无需运行**——它被现有数据直接否证，节省了一次大规模 rollout；
2. **动作效应是 (state, action) 交互性的、而非参数幅度驱动的**：结果差异的存在
   （Phase 2.8 测得 SNR 4.8）不来自"动作变化大"，因此无法通过放大动作差异来提升可辨识性；
3. **与 AUC 0.52 相互印证**：若对比度决定可区分性，AUC 应随对比度的分布而改善；
   实测既无对比度依赖、AUC 又停在 0.52，说明**可辨识性的缺失是结构性的**。

至此，Phase 2.8/2.9 提出的三条剩余杠杆（排序目标、交互表示、极端对比干预）
**全部被证据排除**，剩下的只有"改变整个问题设定"（更长 horizon / 更多重复 /
不同状态表示），而那已超出当前计划的边界。
